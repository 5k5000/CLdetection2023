...
class CephalometricDataset_ISBI2015(BaseCocoStyleDataset):
    ...
    METAINFO: dict = dict(from_file='configs/_base_/datasets/cephalometric_isbi2015.py')
    ...
    def __init__(self,
                 ...
                 use_outer_bbox: bool = False, # wuqian加的 用来控制是否使用所有keypoint的外包矩形来作为bbox，这里的外包矩形会扩大一些,  这是为了检测模型设计的 （在关键点定位模型前增加一个检测模型）
                 ):
