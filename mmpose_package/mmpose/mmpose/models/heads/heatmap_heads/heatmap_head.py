...
class HeatmapHead(BaseHead):
    ...
    def __init__(self,
        ...
        self.specified_target_class = specified_target_class
    ...
    def loss(self,
        ...
        """
        解释 这里keypoints weights 是用来指示关键点是否可见(visible)的
        """
        # print(keypoint_weights.shape)       # [B,K]
        # print(keypoint_weights.unique())    # [0,1]

        if self.specified_target_class is not None:
            specified_target_class = torch.ones_like(keypoint_weights,dtype=torch.float32)
            specified_target_class = specified_target_class / 10
            # specified_target_class =
            specified_target_class[:, self.specified_target_class-1] = 10        # 只将指定的类别的关键点的权重置为1
            keypoint_weights = keypoint_weights * specified_target_class


        # calculate losses
        losses = dict()
        # print(keypoint_weights.shape)
...
@MODELS.register_module()
class HeatmapHead_withSigmoid(HeatmapHead):
    def forward(self, feats: Tuple[Tensor]) -> Tensor:
            """Forward the network. The input is multi scale feature maps and the
            output is the heatmap.
            ...
            # print(x.shape)
            x = self.final_layer(x)
            x = torch.sigmoid(x)        # add by Fivethousand
