...
@MODELS.register_module()
class KeypointL1Loss(KeypointMSELoss):
    def forward(self,
                output: Tensor,
                target: Tensor,
                target_weights: Optional[Tensor] = None,
        ...
        if _mask is None:
            ...
            _loss = F.l1_loss(output, target, reduction='none')
            loss = (_loss * _mask).mean()
...
@MODELS.register_module()
class FocalHeatmapLoss_5k(FocalHeatmapLoss):
    """A class for calculating the modified focal loss for heatmap prediction.
    ...
    def __init__(self,
                 ...
                 skip_empty_channel: bool = False,
                 loss_weight: float = 1.0,
                 pos_neg_sep_value: float = 1.0):       # positive negative seperation value
        super(FocalHeatmapLoss_5k, self).__init__(alpha,beta,use_target_weight,
                                               skip_empty_channel, loss_weight)
        self.pos_neg_sep_value = pos_neg_sep_value
    ...
    def forward(self,
        ...
        pos_inds = target.eq(self.pos_neg_sep_value).float()
        neg_inds = target.lt(self.pos_neg_sep_value).float()
...
@MODELS.register_module()
class FocalLoss(FocalHeatmapLoss):
    def __init__(self):
    ...
    def forward(self,
        ...
        # clip predict heatmap range to prevent loss from becoming nan | 由于log2操作，限制热图预测值范围，防止loss NAN
        predict = torch.clamp(output, min=1e-4, max=1-1e-4)
...
@MODELS.register_module()           # copied from SRPose
    ...
    def forward(self, output, target, target_weight):
        ...
        for idx in range(num_joints):
            ...
            if self.use_target_weight:
                ...
                loss += self.criterion(heatmap_pred * target_weight[:, idx].unsqueeze(1),
