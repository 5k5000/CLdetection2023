...
@MODELS.register_module()
class MultipleLossWrapper_5k(nn.Module):
    """
    我基于MultipleLossWrapper改的,似乎原来的mmpose这个库没有这个功能，没有办法将多个loss相加起来，我在这里实现这个功能
    """
    ...
    def forward(self, input, target, keypoint_weights=None):

        losses = 0
        for i in range(self.num_losses):
            loss_i = self.loss_modules[i](input, target, keypoint_weights)
